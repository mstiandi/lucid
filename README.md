...
`all_signals.behaviors` 只在拼 prompt 时压副本的话，state 里的数据依然无限增长、撑爆 checkpoint 内存。Joker 用独立的 **compression_node** 在状态层做有界化：behaviors 总数超过阈值（默认 200）时，把最老的行为压成「分组统计摘要」，保留最近 50 条原始记录。

**量化**：300 条行为 → 51 条（摘要 + 最近 50），上下文体积 **-83%**。

### 2. 画像 / 观察分离 + 记忆检索化

Joker 的上下文分三类，用三种机制处理：
